        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="text-slate-400 mt-1">
                Análise detalhada mês a mês com breakdown por funil e closer
              </CardDescription>
            </div>
            <Button
              variant="ghost"
              ...
              {monthlyMetrics.map((month, index) => (
                <div key={index} className="bg-slate-700/30 rounded-lg p-6 border border-slate-600/30">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-xl font-semibold text-white">{month.mes}</h3>
                    <Badge 
                      variant="outline" 
                      className={`text-sm ${
                        month.conversionRate >= 60 ? 'border-green-500 text-green-400' :
                        month.conversionRate >= 50 ? 'border-yellow-500 text-yellow-400' :
                        month.conversionRate >= 40 ? 'border-orange-500 text-orange-400' :
                        'border-red-500 text-red-400'
                      }`}
                    >
                      {month.conversionRate.toFixed(1)}% conversão
                    </Badge>
                  </div>

                  {/* Grid de métricas do mês */}
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                    <div className="bg-blue-500/10 rounded p-3 text-center">
                      ...
                      <p className="text-xl font-bold text-white">{month.totalCalls}</p>
                    </div>
                    <div className="bg-green-500/10 rounded p-3 text-center">
                      <p className="text-xs text-green-400 font-semibold mb-1">Comprou</p>
                      <p className="text-xl font-bold text-white">{month.comprou}</p>
                    </div>
                    <div className="bg-red-500/10 rounded p-3 text-center">
                      <p className="text-xs text-red-400 font-semibold mb-1">Não Comprou</p>
                      <p className="text-xl font-bold text-white">{month.naoComprou}</p>
                    </div>
                    <div className="bg-orange-500/10 rounded p-3 text-center">
                      <p className="text-xs text-orange-400 font-semibold mb-1">No Show</p>
                      <p className="text-xl font-bold text-white">{month.noShow}</p>
                    </div>
                  </div>

                  {/* Funis do mês */}
                  {month.funnels && month.funnels.length > 0 && (
                    <div className="mb-4">
                      <h4 className="text-sm font-semibold text-slate-300 mb-3 uppercase">Por Funil</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        {month.funnels.map((funnel: any, fIndex: number) => (
                          <div key={fIndex} className="bg-slate-800/50 rounded p-3 border border-slate-600/30">
                            <p className="text-xs text-slate-400 font-medium mb-2">{funnel.funil}</p>
                            <div className="flex justify-between items-center mb-2">
                              <span className="text-xs text-slate-400">Calls:</span>
                              <span className="text-sm font-semibold text-white">{funnel.totalCalls}</span>
                            </div>
                            <div className="flex justify-between items-center mb-2">
                              <span className="text-xs text-green-400">✓ Comprou:</span>
                              <span className="text-sm font-semibold text-green-400">{funnel.comprou}</span>
                            </div>
                            <div className="flex justify-between items-center mb-2">
                              <span className="text-xs text-red-400">✗ Não comprou:</span>
                              <span className="text-sm font-semibold text-red-400">{funnel.naoComprou}</span>
                            </div>
                            <div className="flex justify-between items-center mb-2">
                              <span className="text-xs text-orange-400">◯ No Show:</span>
                              <span className="text-sm font-semibold text-orange-400">{funnel.noShow}</span>
                            </div>
                            <div className="mt-2 pt-2 border-t border-slate-600/30">
                              <div className="flex justify-between items-center">
                                <span className="text-xs text-purple-400 font-semibold">Conversão:</span>
                                <span className="text-sm font-bold text-purple-400">{funnel.conversionRate.toFixed(1)}%</span>
                              </div>
                            </div>
                          </div>
                        ))}
                  ...
                  {/* Closers do mês */}
                  {month.closers && month.closers.length > 0 && (
                    <div>
                      <h4 className="text-sm font-semibold text-slate-300 mb-3 uppercase">Por Closer</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {month.closers.map((closer: any, cIndex: number) => (
                          <div key={cIndex} className="bg-slate-800/50 rounded p-3 border border-slate-600/30">
                            <p className="text-xs text-slate-400 font-medium mb-2">{closer.closer}</p>
                            <div className="flex justify-between items-center mb-2">
                              <span className="text-xs text-slate-400">Calls:</span>
                              <span className="text-sm font-semibold text-white">{closer.totalCalls}</span>
                            </div>
                            <div className="flex justify-between items-center mb-2">
                              <span className="text-xs text-green-400">✓ Comprou:</span>
                              <span className="text-sm font-semibold text-green-400">{closer.comprou}</span>
                            </div>
                            <div className="flex justify-between items-center mb-2">
                              <span className="text-xs text-red-400">✗ Não comprou:</span>
                              <span className="text-sm font-semibold text-red-400">{closer.naoComprou}</span>
                            </div>
                            <div className="flex justify-between items-center mb-2">
                              <span className="text-xs text-orange-400">◯ No Show:</span>
                              <span className="text-sm font-semibold text-orange-400">{closer.noShow}</span>
                            </div>
                            <div className="mt-2 pt-2 border-t border-slate-600/30">
                              <div className="flex justify-between items-center">
                                <span className="text-xs text-purple-400 font-semibold">Conversão:</span>
                                <span className="text-sm font-bold text-purple-400">{closer.conversionRate.toFixed(1)}%</span>
                              </div>
                            </div>
                          </div>
                        ))}
