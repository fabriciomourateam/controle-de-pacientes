...
export function CurrentDataInput({ telefone, nome, onSuccess, editMode = false, open: externalOpen, onOpenChange: externalOnOpenChange }: CurrentDataInputProps) {
  ...
  const [cinturaAtual, setCinturaAtual] = useState('');
  const [quadrilAtual, setQuadrilAtual] = useState('');
  ...
  // Carregar dados existentes sempre que o modal abrir
  useEffect(() => {
    if (open) {
      loadExistingData();
    } else {
      // Limpar apenas os arquivos novos quando fechar, mas manter os dados carregados
      setFotoFrente(null);
      setFotoLado(null);
      setFotoLado2(null);
      setFotoCostas(null);
  ...
  const loadExistingData = async () => {
    try {
      ...
      if (patientData) {
        ...
        // Carregar medidas - priorizar dados atuais, mas se não existir, usar dados iniciais como referência
        ...
        // Priorizar dados atuais, mas se não existir, usar dados iniciais como referência
        if (patient.peso_atual) {
          setPesoAtual(patient.peso_atual.toString());
        } else if (patient.peso_inicial) {
          setPesoAtual(patient.peso_inicial.toString());
        }
        
        if (patient.altura_atual) {
          setAlturaAtual(patient.altura_atual.toString());
        } else if (patient.altura_inicial) {
          setAlturaAtual(patient.altura_inicial.toString());
        }
        
        if (patient.medida_cintura_atual) {
          setCinturaAtual(patient.medida_cintura_atual.toString());
        } else if (patient.medida_cintura_inicial) {
          setCinturaAtual(patient.medida_cintura_inicial.toString());
        }
        
        if (patient.medida_quadril_atual) {
          setQuadrilAtual(patient.medida_quadril_atual.toString());
        } else if (patient.medida_quadril_inicial) {
          setQuadrilAtual(patient.medida_quadril_inicial.toString());
        }
        
        // Data: usar data atual se existir, senão usar data de hoje
        if (patient.data_fotos_atuais) {
          setDataFotos(patient.data_fotos_atuais);
        } else {
          setDataFotos(getLocalDateString());
        }

        // Carregar previews das fotos existentes (apenas fotos atuais, não iniciais)
        if (patient.foto_atual_frente) setPreviewFrente(patient.foto_atual_frente);
        if (patient.foto_atual_lado) setPreviewLado(patient.foto_atual_lado);
        if (patient.foto_atual_lado_2) setPreviewLado2(patient.foto_atual_lado_2);
        if (patient.foto_atual_costas) setPreviewCostas(patient.foto_atual_costas);
      }
    } catch (error) {
  ...
  const handleSubmit = async () => {
    try {
      setLoading(true);
      console.log('🚀 Iniciando salvamento dos dados atuais...');
      ...
      // Atualizar o paciente com os dados atuais
      const updateData: any = {
        data_fotos_atuais: dataFotos
      };

      // Só atualizar fotos se houver novas fotos (arquivos File)
      if (fotoFrenteUrl) updateData.foto_atual_frente = fotoFrenteUrl;
      if (fotoLadoUrl) updateData.foto_atual_lado = fotoLadoUrl;
      if (fotoLado2Url) updateData.foto_atual_lado_2 = fotoLado2Url;
      if (fotoCostasUrl) updateData.foto_atual_costas = fotoCostasUrl;
      ...
      if (pesoAtual) updateData.peso_atual = parseFloat(pesoAtual);
      if (alturaAtual) updateData.altura_atual = parseFloat(alturaAtual);
      if (cinturaAtual) updateData.medida_cintura_atual = parseFloat(cinturaAtual);
      if (quadrilAtual) updateData.medida_quadril_atual = parseFloat(quadrilAtual);
      ...
      toast({
        title: 'Sucesso!',
        description: editMode ? 'Dados atualizados com sucesso' : 'Dados atuais cadastrados com sucesso'
      });

      setOpen(false);
      onSuccess();
      
      // Limpar apenas arquivos novos (File objects) que foram selecionados mas não salvos
      // Os dados salvos serão recarregados quando o modal abrir novamente
      setFotoFrente(null);
      setFotoLado(null);
      setFotoLado2(null);
      setFotoCostas(null);
      ...
      toast({
        title: 'Erro',
        description: error?.message || 'Não foi possível salvar os dados atuais',
        variant: 'destructive'
      });
    } finally {
      setLoading(false);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogTrigger asChild>
          {editMode ? (
            <Button 
              ...
              title="Editar dados atuais"
            >
              <Edit className="w-3 h-3" />
            </Button>
          ) : (
            <Button className="gap-2 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white shadow-lg shadow-green-500/30 hover:shadow-green-500/50 transition-all">
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>📸 {editMode ? 'Editar' : 'Adicionar'} Dados Atuais - {nome}</DialogTitle>
          <DialogDescription>
            {editMode ? 'Atualize' : 'Cadastre'} fotos, peso, altura e medidas atuais do paciente
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <Label htmlFor="peso">⚖️ Peso Atual (kg)</Label>
              <Input
                id="peso"
                type="number"
                step="0.1"
            ...
            <div>
              ...
              <Input
                ...
                placeholder="Ex: 85"
                value={cinturaAtual}
                onChange={(e) => setCinturaAtual(e.target.value)}
                className="mt-1"
              />
            </div>
            <div>
              ...
              <Input
                ...
                placeholder="Ex: 95"
                value={quadrilAtual}
                onChange={(e) => setQuadrilAtual(e.target.value)}
                className="mt-1"
              />
            </div>
          </div>
          ...
          <div className="space-y-4">
            <h3 className="font-semibold text-sm">📷 Fotos Atuais</h3>
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            >
              ...
              {loading ? 'Salvando...' : 'Salvar Dados Atuais'}
            </Button>
          </div>
        </div>
      </DialogContent>
