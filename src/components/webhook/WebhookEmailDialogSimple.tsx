...
import { Mail, Loader2, AlertCircle, CheckCircle, X } from 'lucide-react';
import { Alert, AlertDescription } from '@/components/ui/alert';

interface WebhookEmailDialogSimpleProps {
  open: boolean;
  onClose: () => void;
...
export function WebhookEmailDialogSimple({
  open,
  onClose,
  onConfirm,
  webhookType,
  ...
  // Carregar email do usuário logado quando dialog abrir
  useEffect(() => {
    if (open) {
      console.log('🔵 WebhookEmailDialogSimple: Dialog aberto, carregando email do usuário');
      async function loadUserEmail() {
        try {
          const user = await getCurrentUser();
          if (user?.email) {
            console.log('🔵 WebhookEmailDialogSimple: Email carregado:', user.email);
            setUserEmail(user.email);
            setEmail(user.email);
          } else {
            console.warn('🔵 WebhookEmailDialogSimple: Usuário não encontrado');
          }
        } catch (error) {
          console.error('🔵 WebhookEmailDialogSimple: Erro ao carregar email:', error);
        }
      }
      loadUserEmail();
    } else {
  ...
  const handleConfirm = async () => {
    console.log('🔵 WebhookEmailDialogSimple: handleConfirm chamado', { email });
    ...
    setValidating(true);
    console.log('🔵 WebhookEmailDialogSimple: Validando email...');
    ...
    try {
      ...
      if (!user) {
        console.error('🔵 WebhookEmailDialogSimple: Usuário não autenticado');
        toast({
          title: "Erro",
      ...
      console.log('🔵 WebhookEmailDialogSimple: Comparando emails', { 
        inputEmail: email.toLowerCase(), 
        userEmail: user.email?.toLowerCase() 
      });

      if (user.email?.toLowerCase() !== email.toLowerCase()) {
        console.warn('🔵 WebhookEmailDialogSimple: Emails não correspondem');
        toast({
      ...
      console.log('🔵 WebhookEmailDialogSimple: Email válido, confirmando...');
      setLoading(true);
      await onConfirm(email, user.id);
      console.log('🔵 WebhookEmailDialogSimple: Confirmação concluída');
      
    } catch (error: any) {
      console.error('🔵 WebhookEmailDialogSimple: Erro ao confirmar email:', error);
      toast({
        title: "Erro",
  ...
  console.log('🔵 WebhookEmailDialogSimple: Renderizando dialog', { open, userEmail, email, isEmailValid });

  return (
    <Dialog open={open} onOpenChange={(isOpen) => {
      console.log('🔵 WebhookEmailDialogSimple: onOpenChange chamado', { isOpen });
      if (!isOpen) {
        onClose();
      }
    }} modal={true}>
      <DialogContent className="max-w-md bg-slate-900/95 backdrop-blur-sm border-slate-700/50 z-[100]">
        <DialogHeader>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && !loading && !validating && isEmailValid) {
                    handleConfirm();
                  }
                }}
