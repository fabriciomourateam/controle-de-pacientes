...
export function MonthlyGrowthChart({ data, growthMetrics, loading = false }: MonthlyGrowthChartProps) {
  if (loading) {
    return (
      <Card className="bg-gradient-to-br from-slate-800/40 to-slate-900/40 backdrop-blur-sm border-slate-700/50">
        <CardHeader>
          <CardTitle className="text-white">Crescimento Mês a Mês</CardTitle>
          <CardDescription className="text-slate-400">
            Evolução cronológica dos ativos e crescimento
          </CardDescription>
        </CardHeader>
        <CardContent>
  ...
  // Calcular estatísticas do crescimento baseado na ordem cronológica (mes_numero 01-20)
  const totalGrowth = data.length > 1 
    ? ((data[data.length - 1].ativos - data[0].ativos) / data[0].ativos) * 100 
    : 0;

  // Calcular crescimento médio mensal baseado na ordem cronológica
  const monthlyGrowths = [];
  for (let i = 1; i < data.length; i++) {
    const current = data[i];
    const previous = data[i - 1];
    if (previous.ativos > 0) {
      const growth = ((current.ativos - previous.ativos) / previous.ativos) * 100;
  ...
  const positiveMonths = monthlyGrowths.filter(growth => growth > 0).length;
  const totalMonths = monthlyGrowths.length;
    ...
    <Card className="bg-gradient-to-br from-slate-800/40 to-slate-900/40 backdrop-blur-sm border-slate-700/50">
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription className="text-slate-400">
              Evolução cronológica dos ativos
            </CardDescription>
          </div>
          <div className="flex items-center gap-2">
      ...
      <CardContent>
        <div className="space-y-6">
          {/* Gráfico de ativos ao longo do tempo */}
          <div>
            <h4 className="text-sm font-medium text-slate-300 mb-3">Evolução dos Ativos</h4>
        ...
        <div className="mt-6 p-4 bg-slate-800/30 rounded-lg border border-slate-700/50">
          <h4 className="text-sm font-medium text-white mb-3">Estatísticas de Crescimento</h4>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-xs">
            <div>
              <p className="text-slate-400">Crescimento Total:</p>
              <p className={`font-semibold ${totalGrowth >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                {totalGrowth >= 0 ? '+' : ''}{totalGrowth.toFixed(1)}%
              </p>
            </div>
            <div>
              <p className="text-slate-400">Crescimento Médio:</p>
              <p className={`font-semibold ${averageMonthlyGrowth >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                {averageMonthlyGrowth >= 0 ? '+' : ''}{averageMonthlyGrowth.toFixed(1)}%
              </p>
            </div>
            <div>
              <p className="text-slate-400">Meses Positivos:</p>
              <p className="text-green-400 font-semibold">
                {positiveMonths}/{totalMonths}
              </p>
            </div>
            <div>
              <p className="text-slate-400">Ativos Iniciais:</p>
              <p className="text-blue-400 font-semibold">
                {data[0]?.ativos.toLocaleString('pt-BR') || '0'}
              </p>
            </div>
            <div>
              <p className="text-slate-400">Ativos Atuais:</p>
              <p className="text-blue-400 font-semibold">
                {data[data.length - 1]?.ativos.toLocaleString('pt-BR') || '0'}
              </p>
            </div>
          </div>
        </div>
