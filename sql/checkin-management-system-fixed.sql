...
-- Versão corrigida que verifica colunas existentes

-- 1. Verificar e adicionar colunas à tabela checkin para gestão
DO $$ 
BEGIN
    ...
    -- Adicionar coluna notes_count se não existir
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'checkin' AND column_name = 'notes_count') THEN
        ALTER TABLE checkin ADD COLUMN notes_count INTEGER DEFAULT 0;
    END IF;
    
    -- Adicionar coluna updated_at se não existir
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'checkin' AND column_name = 'updated_at') THEN
        ALTER TABLE checkin ADD COLUMN updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
END $$;

-- 2. Atualizar trigger de updated_at se não existir
DO $
BEGIN
...
-- 3. Criar tabela para anotações dos check-ins
...
-- 5. Criar trigger para atualizar updated_at na tabela checkin_notes
DO $
BEGIN
...
-- 6. Função para adquirir lock em um check-in
...
-- 7. Função para liberar lock de um check-in
...
-- 8. Função para limpar locks expirados (executar periodicamente)
...
-- 9. Função para obter membros da equipe disponíveis (integrada com sua estrutura)
...
-- 10. Trigger para atualizar contador de notas
...
-- 12. Políticas RLS para checkin_notes (integradas com team_members)
...
-- 13. Criar view para facilitar consultas de check-ins com informações de equipe
...
-- 15. Atualizar contador de notas para check-ins existentes
...
-- 17. Inserir dados de exemplo para status (se necessário)
