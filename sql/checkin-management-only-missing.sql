-- Sistema de Gestão de Check-ins - Apenas o que está faltando
-- Execute primeiro o script verificar-estrutura-checkin.sql para ver o que já existe

-- PARTE 1: Criar tabela para anotações dos check-ins (se não existir)
...
-- PARTE 3: Criar índices para performance (se não existirem)
...
-- PARTE 4: Criar trigger para updated_at na tabela checkin (se não existir)
...
-- PARTE 5: Criar trigger para updated_at na tabela checkin_notes (se não existir)
...
-- PARTE 6: Função para adquirir lock em um check-in
...
-- PARTE 7: Função para liberar lock de um check-in
...
-- PARTE 8: Função para limpar locks expirados
...
-- PARTE 9: Função para obter membros da equipe disponíveis
...
-- PARTE 10: Função para atualizar contador de notas
...
-- PARTE 11: Criar triggers para contador de notas
...
-- PARTE 12: Habilitar RLS na tabela checkin_notes
ALTER TABLE checkin_notes ENABLE ROW LEVEL SECURITY;

-- PARTE 13: Políticas RLS para checkin_notes
...
-- PARTE 14: Criar view para facilitar consultas
...
-- PARTE 15: Atualizar contador de notas para check-ins existentes (se a coluna existir)
UPDATE checkin SET notes_count = (
    SELECT COUNT(*) FROM checkin_notes cn WHERE cn.checkin_id = checkin.id
) WHERE notes_count IS NULL;

-- PARTE 16: Conceder permissões necessárias
...
-- Finalizado!
SELECT 'Sistema de Gestão de Check-ins (apenas funções) instalado com sucesso!' as resultado;