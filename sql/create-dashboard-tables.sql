-- =====================================================
-- CRIAÇÃO DAS TABELAS PARA DASHBOARD DE MÉTRICAS
-- =====================================================

-- Tabela principal para dados do dashboard
CREATE TABLE IF NOT EXISTS dashboard_dados (
...
CREATE INDEX IF NOT EXISTS idx_dashboard_dados_data_referencia ON dashboard_dados(data_referencia);
CREATE INDEX IF NOT EXISTS idx_dashboard_dados_ano_mes ON dashboard_dados(ano, mes_numero);
CREATE UNIQUE INDEX IF NOT EXISTS idx_dashboard_dados_unique_period ON dashboard_dados(ano, mes_numero);
...
CREATE OR REPLACE VIEW dashboard_saude AS
SELECT 
    *,
...
CREATE INDEX IF NOT EXISTS idx_alertas_dashboard_prioridade ON alertas_dashboard(prioridade);
CREATE INDEX IF NOT EXISTS idx_alertas_dashboard_data ON alertas_dashboard(data_referencia);
CREATE INDEX IF NOT EXISTS idx_alertas_dashboard_ativo ON alertas_dashboard(ativo);

-- View para alertas ativos
CREATE OR REPLACE VIEW alertas_dashboard_ativos AS
SELECT *
...
DROP TRIGGER IF EXISTS update_dashboard_dados_updated_at ON dashboard_dados;
CREATE TRIGGER update_dashboard_dados_updated_at
    BEFORE UPDATE ON dashboard_dados
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_alertas_dashboard_updated_at ON alertas_dashboard;
...
ON CONFLICT (ano, mes_numero) DO NOTHING;
...
COMMENT ON COLUMN dashboard_dados.ativos_total_inicio_mes IS 'Número total de pacientes ativos no início do mês';
COMMENT ON COLUMN dashboard_dados.entraram IS 'Número de pacientes que entraram no mês';
COMMENT ON COLUMN dashboard_dados.sairam IS 'Número de pacientes que saíram no mês';
COMMENT ON COLUMN dashboard_dados.percentual_renovacao IS 'Percentual de renovação de planos';
COMMENT ON COLUMN dashboard_dados.percentual_churn IS 'Percentual de churn (cancelamentos)';

COMMENT ON TABLE alertas_dashboard IS 'Sistema de alertas automáticos para métricas críticas';
COMMENT ON COLUMN alertas_dashboard.tipo IS 'Tipo de alerta: churn_alto, renovacao_baixa, crescimento_negativo, vencimentos_altos';
COMMENT ON COLUMN alertas_dashboard.prioridade IS 'Prioridade do alerta: alta, media, baixa';
...
3. Os dados de exemplo serão inseridos para teste
4. Os alertas serão gerados automaticamente

Para inserir novos dados mensais:
INSERT INTO dashboard_dados (mes, ano, mes_numero, data_referencia, ...) VALUES (...);
