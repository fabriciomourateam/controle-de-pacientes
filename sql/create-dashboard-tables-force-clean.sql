-- =====================================================
-- CRIAÇÃO DAS TABELAS PARA DASHBOARD DE MÉTRICAS - VERSÃO FORCE CLEAN
-- =====================================================

-- Script para FORÇAR a limpeza completa e recriar tudo do zero
-- Este script remove TUDO sem falhar, mesmo que objetos não existam

-- FORÇAR remoção de TODOS os objetos relacionados
DO $$
DECLARE
    r RECORD;
BEGIN
    -- Remover todas as views que dependem de dashboard_dados
    FOR r IN (
        SELECT schemaname, viewname 
        FROM pg_views 
        WHERE schemaname = 'public' 
        AND (viewname LIKE '%dashboard%' OR viewname LIKE '%alertas%')
    ) LOOP
        BEGIN
    ...
    -- Remover todas as tabelas relacionadas
    FOR r IN (
        SELECT schemaname, tablename 
        FROM pg_tables 
        WHERE schemaname = 'public' 
        AND (tablename LIKE '%dashboard%' OR tablename LIKE '%alertas%')
    ) LOOP
        BEGIN
    ...
    FOR r IN (
        ...
        AND (p.proname LIKE '%dashboard%' OR p.proname LIKE '%alertas%')
    ) LOOP
        BEGIN
    ...
    FOR r IN (
        ...
        WHERE n.nspname = 'public' 
        AND (t.tgname LIKE '%dashboard%' OR t.tgname LIKE '%alertas%')
    ) LOOP
        BEGIN
